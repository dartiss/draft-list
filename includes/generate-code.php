...
function adl_draft_list_shortcode( $paras = '', $content = '' ) {
	...
	extract( shortcode_atts( array( 'limit' => '', 'type' => '', 'order' => '', 'scheduled' => '', 'icon' => '', 'folder' => '', 'author' => '', 'template' => '', 'date' => '', 'created' => '', 'modified' => '', 'cache' => '', 'words' => '', 'pending' => '' ), $paras ) );
...
function adl_generate_code( $list_limit = '', $list_type = '', $list_order = '', $scheduled = '', $icon_folder = '', $date_format = '', $created = '', $modified = '', $cache_time = '', $template = '', $words = '', $pending = '' ) {
	...
	if ( ( $cache_time === 'no' ) or ( $cache_time === 'error' ) or ( $list_type === 'error' ) ) {
		...
		$cache_key = 'adl_' . md5( $list_limit . $list_type . $list_order . $scheduled . $icon_folder . $date_format . $created . $modified . $cache_time . $editor . $template . $words . $pending );
	...
	if ( !$code ) {
		...
		if ( !is_numeric( $words ) ) { $code .= adl_report_error( __( 'The minimum number of words is invalid - it must be a number', 'simple-draft-list' ), $plugin_name, false ); $error = true; }		
		...
		if ( strpos( $template, '%draft%' ) === false && strpos( $template, '{{draft}}' ) === false ) { $code .= adl_report_error( __( 'The template must include the {{draft}} tag', 'simple-draft-list' ), $plugin_name, false ); $error = true; }
		...
		if ( !$error ) {
			...
			if ( $scheduled !== 'no' && $scheduled !== false ) { $status .= " OR post_status = 'future'"; }
			if ( $pending ) { $status .= " OR post_status = 'pending'"; }
			...
			if ( ( $words > 0 ) or ( strpos( $template, '%words%' ) !== false ) or ( strpos( $template, '{{words}}' ) !== false ) or ( strpos( $template, '%chars%' ) !== false ) or ( strpos( $template, '%chars+space%' ) !== false ) or ( strpos( $template, '{{chars+space}}' ) !== false ) ) {
			...
			if ( $drafts ) {
				...
				if ( ( ( substr( $template, 0, 6 ) === '{{ol}}' ) or ( substr( $template, 0, 6 ) === '{{ul}}' ) ) && ( $list_limit !== 1 ) ) {
				...
				foreach ( $drafts as $draft_data ) {
					...
					if ( $count && $post_length <= $words ) { $enough_words = false; } else { $enough_words = true; }
					...
					if ( ( $date_accept ) && ( $enough_words ) && ( $draft_title != '' ) && ( strtolower( get_post_meta( $post_id, 'draft_hide', true ) ) != 'yes' ) && ( ( $list_limit == 0 ) or ( $valid_draft <= $list_limit ) ) ) {
					   ...
					   $this_line = str_replace( '{{author+link}}', $author_link, $this_line );					   
						...
						if ( $count ) {
							if ( strpos( $this_line, '%words%' ) !== false ) { $this_line = str_replace( '%words%', number_format( $word_count ), $this_line ); }
							if ( strpos( $this_line, '{{words}}' ) !== false ) { $this_line = str_replace( '{{words}}', number_format( $word_count ), $this_line ); }							
		...
		if ( ( $cache_key !== 'no' ) && ( !$error ) ) { set_transient( $cache_key, $code, HOUR_IN_SECONDS * $cache_time ); }
	}
